...
const ContactsTable: React.FC<ContactsTableProps> = ({
  ...
  const columns: Column<ContactDTO>[] = useMemo(() => [
    ...
    { id: 'company', header: 'Empresa', accessorKey: 'companyName', sortable: true, width: '160px', hideOnMobile: true, cell: ({ row }) => ( <div className="text-sm text-app-gray-200"> {row.companyName ? ( <div className="flex items-center"> <Building2 className="h-3 w-3 mr-1 text-app-gray-400 flex-shrink-0" /> <span className="truncate">{row.companyName}</span> </div> ) : ( <span className="text-app-gray-500 italic">Sin empresa</span> )} </div> ), },
